...
```{r stats anova}
nest <- aov(nav_data$Distance ~ factor(nav_data$Block_num) / factor(nav_data$Target_Name))
summary(nest)
```
...
# Nested ANOVA with Mixed Effects Model

https://rcompanion.org/rcompanion/d_07.html

```{r}
nav_data$Target_Name = as.factor(nav_data$Target_Name)
nav_data$Block_num = as.factor(nav_data$Block_num)
library(nlme)
```

```{r}
model = lme(Distance ~ Block_num, random=~1|Target_Name, data=nav_data, method="REML")
anova.lme(model, type="sequential", adjustSigma = FALSE)

```
```{r}
# Post-hoc comparison of means
posthoc = glht(model, linfct = mcp(Block_num="Tukey"))
